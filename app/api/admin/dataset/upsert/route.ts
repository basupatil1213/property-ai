...
//         documents = await loader.load(); // Ensure this method is correct
...
export async function POST() {
    ...
    try {
        ...
        const filePath = "/Users/basavarajpatil/Developer/Headstarter Fellowship/weekly-projects/property-ai/documents/TrainingData.csv";
        const fileContent = fs.readFileSync(filePath, 'utf-8');

        documents = parse(fileContent, {
            columns: true,
            skip_empty_lines: true
        });

        documents = documents.map((row:any, idx:number) => ({
            pageContent: JSON.stringify(row),
            metadata: { id: idx + 1, source : filePath },
            id : idx + 1,
        }));
